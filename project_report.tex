% ======================================================================
% Development and Research Report for compass_sim -- Report V51
% Updated after creation of compassV2.py
% Compile with: pdflatex project_report_updated.tex (twice for TOC)
% ======================================================================
...
\large Report version V51 -- \code{compassV2.py} research branch}
...
\end{scope}
\end{tikzpicture}
\caption{Idealized geometry of the compass needle used by \code{compassV2.py}, based on the uploaded photographs. The actual dynamics keeps only the pivot position and angle, but the dimensions of the lamina and pivot are used to motivate $I$, $m$, and the optional pivot correction.}
...
\caption{Square-lattice geometry used by \code{compassV2.py}. Pivots lie on a rectangular grid with four nearest neighbours in the bulk. This is the least frustrated of the three default geometries and provides the natural baseline for interpreting domains, hysteresis loops, and avalanche activity. Because the principal lattice directions are orthogonal, field sweeps along $x$ or $y$ have a simple relation to rows and columns of needles.}
...
A critical correction in \code{compassV2.py} is the explicit separation between a physical cutoff in metres and a cutoff in nearest-neighbour shells:
\begin{equation}
...
% ======================================================================
\section{Research branch: \code{compassV2.py}}
\label{sec:compassv2}
% ======================================================================
...
\code{compassV2.py} was created as a research-oriented branch. It is not intended to replace every animation feature of \code{compass.py}; instead it prioritizes reproducibility, numerical clarity, efficient sweeps, and rich time-series output.
...
\caption{Main research observables in \code{compassV2.py}.}\label{tab:v2obs}\\
\toprule
...
\bottomrule
\endfoot
$M_x$, $M_y$, $M_{\rm abs}$ & Net polar magnetization components and magnitude. Useful for conventional hysteresis and relaxation.\\
...
Each V2 run writes a tagged set of outputs:
\begin{itemize}[leftmargin=*]
\item a CSV time series with observables;
\item a JSON metadata file with physical parameters, cutoff, geometry, seed, $Q$, $B_{\rm ref}$, and execution choices;
\item a compressed \code{.npz} final-state file containing positions, angles, angular velocities, and relevant final observables;
\item optional diagnostic plots when \code{--make\_plot} is used.
\end{itemize}

This is a necessary step toward reproducible research. The CSV alone is not sufficient because analysis must know how $m$, $I$, $Q$, $r_{\rm cut}$, and $B_{\rm ext}$ were obtained.
...
python3 compassV2.py \
  --out_dir ~/results/compassV2_smoke \
...
\subsection{Main hysteresis campaign using \code{compassV2.py}}
...
\begin{lstlisting}[style=bashstyle]
python3 compassV2.py \
  --out_dir ~/results/law3m_2026_v2/main_hysteresis \
...
\begin{lstlisting}[style=bashstyle]
python3 compassV2.py \
  --out_dir ~/results/law3m_2026_v2/forc_subset \
...
\begin{lstlisting}[style=bashstyle]
python3 compassV2.py \
  --out_dir ~/results/law3m_2026_v2/step_relax \
...
\begin{lstlisting}[style=bashstyle]
python3 compassV2.py \
  --out_dir ~/results/law3m_2026_v2/pulse_memory \
...
\subsection{Research-grade refactoring: \code{compassV2.py}}
...
\item reproducible metadata and final-state snapshots;
...
\item \textbf{Wrapper scripts must be updated to V2.} The old \code{damping\_sweepV02.py} architecture is useful, but the production wrapper should call \code{compassV2.py} and use the new CSV/metadata columns.
...
\item Create \code{damping\_sweepV03.py} or \code{compassV2\_sweep.py}, preserving V02 parallelism and resume but using V2 outputs.
...
% ======================================================================
\section{Appendix A: \code{compassV2.py} command-line reference}
\label{app:cli}
% ======================================================================

\begin{longtable}{@{}p{4.4cm}p{10.4cm}@{}}
\caption{Main command-line parameters of \code{compassV2.py}.}\label{tab:cli}\\
\toprule
...
\bottomrule
\endfoot
\code{--geometry} & \code{square}, \code{triangular}, or \code{honeycomb}.\\
\code{--N}, \code{--M} & Array dimensions or nominal honeycomb dimensions.\\
\code{--R} & Circumscribed radius; nearest-neighbour distance is approximately $2R$.\\
\code{--needle\_frac} & Needle length as a fraction of $2R$.\\
\code{--moment}, \code{--inertia} & Explicit overrides for magnetic moment and moment of inertia.\\
\code{--needle\_thickness}, \code{--steel\_density}, \code{--steel\_Ms}, \code{--steel\_Bsat} & Parameters for automatic physical derivation of $m$ and $I$.\\
\code{--pivot\_radius}, \code{--pivot\_thickness}, \code{--pivot\_density}, \code{--pivot\_mass} & Optional pivot contribution to inertia and volume correction.\\
\code{--damping}, \code{--damping\_noise} & Central viscous damping and random per-needle relative variation.\\
\code{--t\_sim}, \code{--dt\_factor}, \code{--log\_every} & Simulation time, time step as a fraction of $T_0$, and output cadence.\\
\code{--flip\_angle\_deg} & Threshold for the field-independent large-rotation activity counter.\\
\code{--cutoff\_shells}, \code{--cutoff\_m} & Dipolar cutoff in units of $r_{\rm nn}$ or as an absolute length.\\
\code{--pbc}, \code{--n\_images} & Periodic boundary conditions and number of image cells.\\
\code{--tensor\_mem\_limit\_gb} & Memory limit for precomputed dipolar tensor.\\
\code{--float32} & Single precision for exploratory runs. Double precision remains preferred for final results.\\
\code{--field\_mode} & \code{static}, \code{hysteresis}, \code{forc}, \code{sine}, \code{step\_up}, \code{step\_down}, \code{pulse}, \code{demag\_rot}, or \code{demag\_linear}.\\
\code{--B\_ext}, \code{--B\_max\_factor}, \code{--phi\_ext\_deg} & Applied field amplitude or automatic amplitude from $B_{\rm ref}$, plus direction.\\
\code{--forc\_n\_curves}, \code{--forc\_Br\_min}, \code{--forc\_rate} & FORC sampling parameters.\\
\code{--field\_delay}, \code{--t\_pulse} & Step and pulse timing.\\
\code{--out\_dir}, \code{--tag} & Output directory and file tag.\\
\code{--use\_gpu}, \code{--progress}, \code{--verbose}, \code{--make\_plot} & Execution and diagnostic options.\\
\end{longtable}
...
The exact column order should be read from the CSV header, but the intended research structure is:

\begin{lstlisting}[style=csvstyle]
t_s,Bx_T,By_T,Bproj_T,Mx,My,Mabs,Mproj,S1,S2,theta_director_rad,
q_axis,omega_rms,omega_max,flip_field,flip_angle,E_dip_J,E_ext_J,
E_kin_J,E_total_J,stage,forc_index,forc_Br_T
\end{lstlisting}

This header is intentionally redundant. Redundancy is useful in research data because it allows independent checks: for example, $M_{\rm abs}$ can be recomputed from $M_x$ and $M_y$, and $B_{\rm proj}$ can be checked against the field direction.
...
V2 research branch & 10/07/2026 & New \code{compassV2.py}: explicit cutoff, new observables, native activity counters, energy ledger, metadata, final-state snapshots, and research-oriented protocol/output design.\\
