...
% Updated after review of compass.py V2.0.2
% Compile with: pdflatex project_report.tex (twice for TOC), run from docs/
% (figure paths are relative to this file's directory: ../figs/...)
% ======================================================================
...
It should be noted that Eq.~\ref{eq:bref} is the \emph{on-axis}
(head-to-tail) field of a point dipole at the nearest-neighbour
distance; the transverse configuration would yield half this value.
$B_{\rm ref}$ is therefore a normalization convention --- an
order-of-magnitude scale for the local dipolar field --- rather than
an exact reference field, and its relation to the true mean local
field depends on the lattice geometry and on the instantaneous
bond-orientation distribution. All quantities expressed in units of
$B_{\rm ref}$ (the auto-selected field amplitude via
\code{--B\_max\_factor}, pulse amplitudes, and reported sweep rates)
...
The equation of motion, Eq.~\ref{eq:eom}, is integrated with a
semi-implicit (damped) velocity-Verlet scheme. Positions are advanced
with the standard Verlet half-step, but the linear damping term
$-b\,\dot\theta_i$ is treated implicitly in the velocity update:
the new angular velocity is obtained by solving the linear relation

\begin{equation}
\omega_i^{\,n+1} \;=\;
\frac{\omega_i^{\,n}\!\left(1-\dfrac{b\,\Delta t}{2I}\right) + \tfrac{\Delta t}{2I}\!\left(\tau_i^{\,n} + \tau_i^{\,n+1}\right)}
     {1 + \dfrac{b\,\Delta t}{2I}},
\label{eq:verlet}
\end{equation}

rather than by evaluating the damping force explicitly. Because the
damping enters through the denominator, the scheme is unconditionally
stable with respect to the damping parameter $b$: arbitrarily strong
damping cannot drive a numerical instability, only over-relax the
dynamics. The time step itself is set from the nearest-neighbour
dipolar frequency scale, $\Delta t = f_{\rm dt}\,T_0$ with
$f_{\rm dt} = 0.04$ by default, and a runtime guard monitors the
instantaneous local field-derived stiffness frequency
$\omega_{i,\rm loc}=\sqrt{m|B_i|/I}$ (not the needle's own angular
velocity) through $\omega_{i,\rm loc}\,\Delta t$ against a threshold,
to flag (and optionally sub-step) under-resolved intervals. This is
the property exploited by the time-step convergence check of
Section~\ref{sec:validation}: for a resolved run, halving $f_{\rm dt}$
must leave loop metrics and avalanche statistics unchanged within
...
Two caveats apply to this truncation. First, the dipolar interaction
decays as $1/r^3$, which in two dimensions is only conditionally
convergent: lattice sums of dipolar energies are in general
shape- and boundary-dependent, in direct analogy with demagnetizing
fields in continuum magnetostatics. The present implementation applies
a sharp real-space cutoff with no Ewald-type or shape-aware
correction. Second, and as a consequence, the cutoff-convergence check
of Section~\ref{sec:validation} (repeating runs at
$r_{\rm cut}/r_{\rm nn} = 2.5,\ 3.5,\ 5$) probes \emph{sensitivity} to
the cutoff radius but cannot exclude a systematic contribution common
to all truncated sums, i.e.\ the long-range tail that a full Ewald
treatment would recover. For observables dominated by short-range
rearrangements (individual flips, small avalanches) this bias is
expected to be weak; for collective, system-spanning quantities
(demagnetized ground-state selection, large-avalanche cutoffs,
finite-size scaling) it should be kept in mind as a methodological
limitation, or addressed in future work by an Ewald-summed or
brute-force all-pairs reference run.
...
The V2 output is designed so that analysis does not need to reconstruct physics from only $M(t)$ and $B(t)$. The standard CSV now includes the following classes of observables (Table~\ref{tab:v2obs}):
...
$E_{\rm dip}$, $E_{\rm ext}$, $E_{\rm kin}$, $E_{\rm total}$ & Energy ledger ($E_{\rm kin}$ is the rotational kinetic energy $\tfrac12 I\sum_i\omega_i^2$, logged as the \code{E\_kin\_J} CSV column). Needed to interpret relaxation pathways, dissipation, and numerical stability.\\
...
Each V2.0.2 run writes a tagged directory tree under \code{--out\_dir}. The current implementation creates four subdirectories (the exact CSV column order is given in Appendix~\ref{app:csv}):
\begin{itemize}[leftmargin=*]
...
The metadata records the program name, version, \code{source\_file\_timestamp}, \code{created\_unix\_time}, \code{created\_datetime\_utc}, \code{created\_datetime\_local}, the effective configuration, derived quantities such as $N_{\rm needles}$, $r_{\rm nn}$, $B_{\rm ref}$, $B_{\rm eff}$, $\omega_0$, $T_0$, $\Delta t$, $Q$, cutoff policy, tensor memory, numerical dtype, final domain statistics, and runtime. The CSV alone is therefore not treated as a complete scientific record; interpretation also requires the JSON metadata and the state snapshots.
...
The main computational cost is the dipolar field, which scales as $N_{\rm needles}^2$ for $N_{\rm needles}$ needles. Since needle positions are fixed, V2 uses a precomputed interaction tensor when memory allows. The field is then obtained from matrix-vector products rather than rebuilding all pair separations at every time step. This is efficient on CPU through BLAS and can also benefit from GPU acceleration for sufficiently large arrays.
...
Each study below probes the damped Newtonian dynamics of Eq.~\ref{eq:eom} through one of the field protocols of Section~\ref{sec:compassv2}, and no result from any of them should be interpreted physically until the corresponding checks in the Validation Plan (Section~\ref{sec:validation}) have passed.
...
\textbf{Critical analysis.} Do not rely on only $dM/dB$ thresholding. Use the native flip counters as primary activity measures. Fit power laws only after checking tail size, cutoff stability, and alternatives such as log-normal distributions~\cite{clauset2009}. Report when statistics are insufficient.
...
The example commands below use only a subset of the available flags; see Appendix~\ref{app:cli} (Table~\ref{tab:cli}) for the full command-line reference.
...
The narrative below is condensed into the version table of Appendix~\ref{app:versions} (Table~\ref{tab:versions}).
...
\caption{Main command-line parameters of \code{compass.py}, version 2.1.0 (as of commit \texttt{d1a9895}). Flag names and default values in this table are checked against \code{build\_parser()} by \code{tools/generate\_cli\_reference.py}; see the note at the end of this appendix.}\label{tab:cli}\\
\toprule
...
\code{--needle\_len}, \code{--needle\_width}, \code{--needle\_thickness} & Physical dimensions of the rhombus-shaped needle blade. Defaults are \SI{10}{mm}, \SI{3}{mm}, and \SI{0.4}{mm} (the ``LAW3M large needle'' profile; this differs on purpose from the \SI{0.26}{mm} ``real small needle'' prototype of Figure~\ref{fig:needle_geometry}, which motivates \code{damping\_sweep.py}'s defaults instead).\\
...
\code{--log\_adaptive} & If $>0$, also logs a row whenever $|\Delta M_{\rm proj}|$ since the last logged row exceeds this threshold, or a flip event just committed, in addition to the \code{--log\_every} cadence. $0.0$ (default) reproduces the \code{--log\_every} cadence exactly.\\
\code{--flip\_angle\_deg} & Threshold for the field-independent large-rotation activity counter.\\
\code{--flip\_band\_deg}, \code{--flip\_dwell\_T0}, \code{--flip\_settle\_frac} & Parameters of the V2.1 hardened, ringing-proof \code{flip\_field}/\code{flip\_angle} avalanche counters (Schmitt-trigger dead-band half-width around the perpendicular to the drive axis; dwell time, in units of $T_0$, required before a state change is committed; and the $|\omega|$ settling threshold, as a fraction of $\omega_0$, for the rest-angle channel). Defaults: \SI{30}{\degree}, $0.5\,T_0$, and $0.05\,\omega_0$.\\
\code{--event\_log} & If set, writes a per-committed-event CSV (\code{step}, \code{t}, \code{needle\_id}, \code{channel}, \code{theta}) for offline spatio-temporal avalanche clustering. Off by default.\\
\code{--dt\_guard\_alpha}, \code{--dt\_guard\_substep} & V2.1 numerical-stability monitor: an integration step is flagged when the local field-derived stiffness frequency $\sqrt{m|B_i|/I}$ times $\Delta t$ exceeds the threshold \code{--dt\_guard\_alpha} (default $0.35$). \code{--dt\_guard\_substep} optionally re-integrates flagged steps with 4 global sub-steps (off by default; breaks strict symplecticity, so treat as exploratory).\\
...
\code{--hyst\_slow\_window}, \code{--hyst\_slow\_factor} & Optional \code{"B\_lo,B\_hi"} window (both branches, applied to $|B|$) within which the sweep rate is divided by \code{--hyst\_slow\_factor}; implemented as exact piecewise-linear \code{HystSchedule} legs (Section~\ref{sec:integrator} and \code{FORC.md} \S5), with the full per-leg schedule recorded in metadata. Unset by default (base rate everywhere, identical to the pre-existing schedule).\\
...
\noindent\textit{On keeping this table current.} \code{tools/generate\_cli\_reference.py} introspects \code{build\_parser()} directly and can print every flag's name, default value, and \code{argparse} help string; running it (or its \code{--check} mode) is the fastest way to detect a drift like the one this appendix previously had (the six V2.1 flags above were missing entirely). Full automatic regeneration of this table was evaluated but not adopted wholesale: roughly a third of the flags in \code{build\_parser()} have no \code{help=} string in the source, so a purely mechanical dump would replace several of the descriptions above with a placeholder. The flag names and default values above are verified against the script's output; the prose descriptions for flags lacking a source \code{help=} string remain hand-authored and could in principle drift again if the underlying behavior changes without updating this table.
...
V2.0.2 & 10/07/2026 & Source timestamp added; metadata now records source and run creation times; the simulation saves initial and final NPZ state files and always generates initial, final, and side-by-side lattice PNGs.\\
Report V52 & 10/07/2026 & Updated the report to match the \code{compass.py} V2.0.2 implementation, including exact output structure, CSV header, PNG options, and timestamp metadata.\\
V2.1 / V2.2 generation & 12/07/2026 & Hardened, ringing-proof \code{flip\_field}/\code{flip\_angle} avalanche counters (Schmitt-trigger dead-band plus dwell-time debouncing on the drive-axis channel; settle-then-commit debouncing on the rest-angle channel); a numerical-stability guard monitoring the local field-derived stiffness frequency against $\Delta t$, with an optional global 4-way sub-stepping correction; and optional per-committed-event CSV logging (\code{--event\_log}) for offline spatio-temporal avalanche clustering.\\
Report V53 & 12/07/2026 & Updated the report to describe the current canonical engine (version 2.1.0, git commit \texttt{d1a9895}) rather than the V2.0.2 snapshot: replaced the hardcoded source-file timestamp in the abstract with git-based provenance, added the six V2.1 flags to Appendix~\ref{app:cli}, resolved the $K$/$K_{\rm rot}$ notation collision, and added this row.\\
\end{longtable}
