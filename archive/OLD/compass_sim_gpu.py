...
def relax(thetas, xs, ys, t_sim=2.0, dt_factor=0.05,
    ...
    # ── Pré-computação das matrizes de acoplamento dipolar estático ───────
    # Como as posições das agulhas (xs, ys) são fixas, as distâncias relativas
    # e os coeficientes de interação são constantes durante toda a simulação.
    # Pré-calculamos as matrizes Cxx, Cxy, Cyy de tamanho (K, K) uma única vez
    # no backend ativo (CPU ou GPU) para evitar loops e alocações no loop principal.
    K = N * M
    x_flat_xp = _local_to_backend(xs.ravel())
    y_flat_xp = _local_to_backend(ys.ravel())

    Cxx_xp = _active_xp.zeros((K, K))
    Cxy_xp = _active_xp.zeros((K, K))
    Cyy_xp = _active_xp.zeros((K, K))
    ...
    for dx_img in x_shifts:
        for dy_img in y_shifts:
            # broadcasting: x_flat_xp[:, None] (receptor), x_flat_xp[None, :] (fonte)
            rx = (x_flat_xp[:, None] - x_flat_xp[None, :]) - dx_img
            ry = (y_flat_xp[:, None] - y_flat_xp[None, :]) - dy_img
            r2 = rx*rx + ry*ry

            is_self = (r2 < 1e-24)
            valid = (~is_self) & (r2 <= cutoff*cutoff)

            # evita divisão por zero nos elementos mascarados
            r2_safe = _active_xp.where(valid, r2, 1.0)
            r = _active_xp.sqrt(r2_safe)
            r5 = r2_safe * r2_safe * r

            # coeficientes analíticos da interação dipolar
            c_xx = MU0_OVER_4PI * (3.0 * rx * rx / r5 - 1.0 / (r2_safe * r))
            c_xy = MU0_OVER_4PI * 3.0 * rx * ry / r5
            c_yy = MU0_OVER_4PI * (3.0 * ry * ry / r5 - 1.0 / (r2_safe * r))

            c_xx = _active_xp.where(valid, c_xx, 0.0)
            c_xy = _active_xp.where(valid, c_xy, 0.0)
            c_yy = _active_xp.where(valid, c_yy, 0.0)
    ...
    def _torques_xp(theta_flat_xp, bx_ext, by_ext):
        """
        Versão GPU-resident otimizada: Calcula os torques no plano usando
        multiplicações de matriz-vetor no backend ativo (_xp), sem alocações
        e de forma extremamente veloz.
        """
        mx = moment * _active_xp.cos(theta_flat_xp)
        my = moment * _active_xp.sin(theta_flat_xp)

        # B_dipolar = C @ m
        Bx_tot = Cxx_xp @ mx + Cxy_xp @ my + bx_ext
        By_tot = Cxy_xp @ mx + Cyy_xp @ my + by_ext

        # torque: tau_z = mx * By - my * Bx
        tau_flat = mx * By_tot - my * Bx_tot
        return tau_flat

    def _torques(th, bx_ext, by_ext):
        """
        Versão de compatibilidade (CPU->GPU->CPU) para chamadas fora do loop quente.
        """
    ...
    for step in range(1, n_steps + 1):
        ...
        # independente de --progress_bar, usando _print() normal (linha
        # nova) em vez da barra com \r. Isso garante que informação sobre o
        # andamento da simulação continue disponível mesmo quando a barra
        # está desabilitada (ex: saída redirecionada para arquivo de log,
        # onde \r não se comporta como em um terminal interativo).
        ...
        if (_status_now - _last_status_print_time) >= 2.0 or step == n_steps:
            ...
            _print(f"  [passo {step}/{n_steps}]  t={t_now:.4f}s  "
                   f"B={B_status*1e3:+.4f}mT  S={S_now:.4f}  w_max={omega_max:.3f}rad/s")
...
def plot_state(thetas, xs, ys, title="Rede de bussolas", show_order=True,
    ...
    if show_order:
        ...
        _n_domains_found = None   # exposto no retorno só quando halo_mode='domains'
