...
V46 & Alinhamento das molduras do texto de ajuda (\emph{docstring}): as
bordas direitas dos quadros de parâmetros, que serpenteavam em editores
de texto simples por diferença de largura visual (caracteres acentuados,
setas, \emph{box-drawing}), foram normalizadas para largura visual
uniforme; três descrições longas foram levemente encurtadas para caber.
Sem efeito sobre a física ou a execução. \emph{(Versão atual.)} \\
\midrule
...
verificadas contra a versão V45. Os comandos usam \code{compass.py}
como nome do executável --- uma cópia da versão atual do simulador
(\code{compass\_simV45.py}), mantida com esse nome estável para uso
diário; assim os exemplos podem ser copiados e colados diretamente. A
...
V11 & 05/07/2026 & Nos exemplos do Apêndice~B, o executável passou a ser
referido como \code{compass.py} (cópia estável da versão atual), para
cópia e colagem direta. O registro de versões (V01--V45) é preservado
no corpo e nas tabelas. \\
V12 & 05/07/2026 & Documentada a V46 (alinhamento das molduras do texto
de ajuda). \\
\bottomrule
\end{tabular}
